...
import { OSATQualityReportData } from '../types/osat';
import { PurchaseReportData } from '../types';
import { roundTo3Decimals } from './numberUtils';

export const parseGangshanReportExcelFile = async (file: File): Promise<{
  qualityReportData: OSATQualityReportData[];
  ...
  return new Promise((resolve, reject) => {
    ...
    reader.onload = (e) => {
      try {
        ...
        // 創建廠商數據映射
        const qualityDataMap = new Map<string, OSATQualityReportData>();
        ...
        for (let i = dataStartRow; i < qualityJsonData.length; i++) {
          ...
          if (row && row.length >= 18) {
            ...
            if (vendorName && vendorName !== '廠商名稱' && vendorName !== '合計' && vendorName !== '供應商' && vendorName.length > 0) {
              ...
              qualityDataCount++;
              qualityDataMap.set(vendorName, {
                vendorName,
                shipmentQuantity: String(roundTo3Decimals(Number(row[2]) || 0)),  // 出货量（K）- C欄
                ...
                service: roundTo3Decimals(Number(row[20]) || 0),                // 服務-品管鑑定分數(權重5%) - U欄
          ...
          if (row && row.length >= 10) {
            ...
            if (vendorName && vendorName !== '廠商名稱' && vendorName !== '合計' && vendorName !== '供應商' && vendorName.length > 0) {
              ...
              purchaseDataMap.set(vendorName, {
                ...
                totalPurchaseAssessmentScoreA: null,                              // OSAT-採購鑑定分數(權重20%) - 由系統計算
        ...
        // 轉換為陣列
        const qualityReportData = Array.from(qualityDataMap.values());
        const purchaseReportData = Array.from(purchaseDataMap.values());
        
        if (qualityReportData.length === 0) {
          reject(new Error(`未能解析到任何供應商資料。品質數據: ${qualityDataCount}筆, 採購數據: ${purchaseDataCount}筆。請確認檔案格式與數據行結構。`));
          return;
        }
        
        console.log('解析完成:', { 
          qualityCount: qualityReportData.length, 
          purchaseCount: purchaseReportData.length 
        });
